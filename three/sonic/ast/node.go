...
func (self *Node) SetInt(key string, s string) (bool, error) {
	return self.Set(key, NewNumber(s))
}

func (self *Node) SetString(key string, s string) (bool, error) {
	return self.Set(key, NewString(s))
}
...
func (self *Node) GetBool(key string) bool {
	s, err := self.Get(key).Bool()
	if err != nil {
		return false
	}
	return s
...
func (self *Node) GetString(key string) string {
	s, err := self.Get(key).String()
	if err != nil {
		return ""
	}
	return s
...
func (self *Node) GetUInt64(key string) uint64 {
	s, err := self.Get(key).Int64()
	if err != nil {
		return 0
	}
	return uint64(s)

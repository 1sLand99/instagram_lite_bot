...
func HttpRequest(bot *gostruct.BotData) http.Response {
	...
	if bot.HttpRequest.Request.ReadResponseHeaders {
		bot.HttpRequest.Response.Headers = gotools.MapStringSliceToMapString(bot, res.Header)
	}
	...
	if bot.HttpRequest.Request.ReadResponseBody {
		...
		if bot.HttpRequest.Request.GzipCompression {
			source, err := gotools.CompressGzip(string(resp))
			if err != nil {
				bot.HttpRequest.Response.Source = string(resp)
			}
			bot.HttpRequest.Response.Source = string(source)
		} else {

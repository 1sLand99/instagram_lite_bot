...
func main() {
	...
	parseScreen := widget.NewButton("parse screen", func() {
		parse(func(line string) string {
			...
			log.Info("screen_tree1: %s", recver.Tree2String(recver.DumpViewTreeRecvView(screen), false))
			log.Info("screen_tree2: %s", recver.Tree2String(recver.DumpViewTreeRecvView(screen), true))
			marshal, _ := json.Marshal(screen)
			log.Info("screen: %s", marshal)
			return ""
		})
	})

...
func (this *InsLiteClient) OnScreenRecvFinish(screen *recver.ScreenReceived, allSubScreen *recver.SubScreenArray) error {
	if screen.ScreenId == ScreenIdIgCarbonRegistration {
		...
		this.sendNetworkInfo(ScreenIdIgCarbonRegistration, this.getScreenIdByName(this.curScreenName), screen.DecodeBodyDataSize)
	}
	return nil
}
...
func (this *InsLiteClient) recvScreenReceived(msgEvent *net.MsgWrap) error {
	screen := msgEvent.Body.Interface().(*recver.ScreenReceived)
	...
	if screen.GetScreenId() != LoadingScreenId {
		...
		this.sendBrowserAction(screen, &screen.NavigationData,
			true, "", 0, "")
	}

...
func CreateRolaPool(ipType int) *RolaProxy {
	return &RolaProxy{
		Type:            ipType,
		RotatingResiUrl: "http://list.rola.info:8088/user_get_ip_list?token=&qty=1&country=%s&state=&city=&time=10&format=json&protocol=socks5&filter=1",
		PhoneNetUrl:     "http://list.rola.info:8088/user_get_ip_list?token=&type=4g&qty=1&country=%s&time=10&format=json&protocol=socks5&filter=1",
		DatacenterUrl:   "http://list.rola.info:8088/user_get_ip_list?token=&type=datacenter&qty=1&time=10&country=%s&format=json&protocol=socks5&filter=1",
		client:          &http.Client{},
	}
}

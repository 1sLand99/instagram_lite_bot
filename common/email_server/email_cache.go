...
type EmailCache struct {
	cache     map[string]map[string]base.EmailInterface // project->email
...
func (this *EmailCache) SaveNewEmailDb(e base.EmailInterface, project string, provider string) error {
...
func (this *EmailCache) GetEmailByProject(project string) (base.EmailInterface, error) {
	this.cacheLock.Lock()
	defer this.cacheLock.Unlock()

	pjCache := this.cache[project]
	if pjCache == nil {
		pjCache = make(map[string]base.EmailInterface)
		this.cache[project] = pjCache
	}
	if len(pjCache) > 0 {
		var key string
		var value base.EmailInterface
...
func CreateDbCacheEmailProvider(DB *gorm.DB) *EmailCache {
	result := &EmailCache{
		DB:    DB,
		cache: make(map[string]map[string]base.EmailInterface),
	}
